...
// Intercept response send to wait for database writes to complete on Serverless/Vercel environments
app.use((req, res, next) => {
  const originalSend = res.send;
  res.send = function (body?: any) {
    if (pendingWrites.length > 0) {
      const currentPending = [...pendingWrites];
      pendingWrites = []; // Reset queue for next execution on this instance
      Promise.all(currentPending)
        .then(() => {
          originalSend.call(this, body);
        })
        .catch((err) => {
          console.error("Error waiting for pending writes in serverless interceptor:", err);
          originalSend.call(this, body);
        });
    } else {
      originalSend.call(this, body);
...
function readDb() {
  try {
    ...
    if (db && db.settings) {
      ...
      if (process.env.SQL_CONNECTION_URI) {
        db.settings.sqlConnectionUri = process.env.SQL_CONNECTION_URI;
        db.settings.sqlEnabled = true;
        changed = true;
      }
      if (process.env.SQL_DIALECT) {
        db.settings.sqlDialect = process.env.SQL_DIALECT as "postgres" | "mysql";
        changed = true;
      }
      if (changed) {
        fs.writeFileSync(DB_FILE, JSON.stringify(db, null, 2));
...
function writeDb(data: any) {
  try {
    ...
    const firestorePromise = saveDbToFirestore(data).catch((err) => {
